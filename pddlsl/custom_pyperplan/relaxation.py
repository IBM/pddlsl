...
class _RelaxationHeuristic(Heuristic):
    ...
    def __call__(self, node):
        ...
        for fact in state:
            ...
            if fact in self.facts: # If fact is not in self.facts, then it corresponds to a static predicate, so we ignore it
                heapq.heappush(
...
class hFFHeuristic(_RelaxationHeuristic):
    ...
    def calc_h_with_plan(self, node):
        ...
        for fact in state:
            ...
            # If fact is not in self.facts, then fact corresponds to a static predicate, so
            # we simply ignore it
